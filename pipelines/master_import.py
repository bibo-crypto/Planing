...
from utility.file_cache import CACHE_DIR
...
SHEET_CANDIDATES: dict[str, list[str]] = {
    ...
    "biglietti_template": ["biglietti", "forma biglietti"],
}
...
LABELS: dict[str, str] = {
    ...
    "lotti": "LOTTI",
    "listini": "LISTINI",
    "biglietti_template": "Forma Biglietti",
...
def find_files_in_directory(dir_path: str | Path) -> dict[str, Path]:
    ...
    valid_extensions = {".xlsx", ".xls", ".xlsm", ".csv", ".docx"}
...
def _extract_sheet_to_temp(master_path: str, sheet_name: str, persistent: bool = False) -> str:
    """Copy one sheet into its own standalone xlsx.

    A persistent extract is needed when other tabs must reopen a sheet after
    the master import has finished; a short-lived temp file would leave their
    shared cache pointing at a deleted path.
    """
    ...
    try:
        ...
        src_wb.close()
    if persistent:
        extract_dir = CACHE_DIR / "master_import"
        extract_dir.mkdir(parents=True, exist_ok=True)
        tmp_path = str(extract_dir / f"{_norm(sheet_name)}.xlsx")
    else:
...
def _content_match_ordine_sheets(wb, sheetnames: list[str], already_matched: set[str]) -> dict[str, str]:
    """Same content-sniffing idea as ``_content_match_ordine_files``, but for
    sheets inside a single already-open master workbook instead of separate
    files -- Data Ordine / Dispo Bagno have no reliable sheet-name candidate
    (their real name is order-specific), so a name-only match never finds
    them here either.
    """
    found: dict[str, str] = {}
    data_signature = {"descrizioneaggiuntivaordine", "cliente"}
    ...
    for sheet_name in sheetnames:
        if sheet_name in already_matched:
            continue
        try:
            sheet = wb[sheet_name]
            header_sets = [
            ...
            continue
        if "data_ordine" not in found and any(
            data_signature.issubset(headers) or kamal_signature.issubset(headers)
            for headers in header_sets
        ):
            found["data_ordine"] = sheet_name
            continue
        if "dispo_bagno" not in found and any(
            dispo_signature.issubset(headers) for headers in header_sets
        ):
            found["dispo_bagno"] = sheet_name
    return found
...
def import_master_file(master_path: str, situazione_tab, magazino_tab, biglietti_tab=None, prezzi_tab=None) -> tuple[list[str], list[str]]:
    ...
    # Data Ordine / Dispo Bagno never have a matchable sheet name (see
    # _content_match_ordine_sheets), so a single master workbook that
    # bundles them was previously never routing them anywhere and never
    # even reporting them as skipped -- sniff sheet content the same way
    # the directory-import path already does for loose files.
    if "data_ordine" not in matches or "dispo_bagno" not in matches:
        matches.update(_content_match_ordine_sheets(wb, sheetnames, set(matches.values())))
    wb.close()

    loaded: list[str] = []
    # Report every known source that wasn't found, not just the ones with a
    # sheet-name candidate -- densita is reported separately below (it's
    # matched by a two-sheet signature, not a single sheet name), so it's
    # excluded here to avoid double-reporting it before that block runs.
    skipped: list[str] = [LABELS[k] for k in LABELS if k not in matches and k != "densita"]

    for key, sheet_name in matches.items():
        tmp_path = None
        try:
            # Keep the extracted source available to every tab that reloads
            # from the shared cache after this import returns.
            tmp_path = _extract_sheet_to_temp(master_path, sheet_name, persistent=True)
            _route(key, tmp_path, tmp_path, situazione_tab, magazino_tab, biglietti_tab, prezzi_tab)
    ...
    if _norm("Entry") in normalized and _norm("Densita") in normalized:
        try:
            ...
            from utility.densita_cache import save_densita_cache
...
def _route(
    ...
    if key in ("dfm", "copertura", "data_prod", "wincoint", "uscita", "qualita"):
        ...
        if situazione_tab is not None:
            ...
            from utility.articoli_cache import save_articoli_cache
        ...
        if situazione_tab is not None:
            ...
            from utility.prezzi_cache import save_prezzi_cache
    ...
    elif key == "biglietti_template":
        if biglietti_tab is not None and hasattr(biglietti_tab, "set_template_path"):
            biglietti_tab.set_template_path(tmp_path)
