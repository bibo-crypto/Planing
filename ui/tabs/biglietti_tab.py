...
def _lazy_exporter_call(function_name):
    ...
    def call(*args, **kwargs):
        from biglietti_exporter import __dict__ as exporter_namespace
...
class BigliettiTab(ttk.Frame):
    ...
    def _build_folder_row(self, parent, row_idx, kind, btn_text, with_checkbox=False):
    ...
    def _worker_convert(self):
        ...
        try:
            ...
            if not template.is_file():
                ...
                if local_tmpl.is_file(): template = local_tmpl
            if not template.is_file(): raise ValueError("Template 'Biglietti.docx' not found on Desktop.")
            if order_format == "EL_KAMAL":
